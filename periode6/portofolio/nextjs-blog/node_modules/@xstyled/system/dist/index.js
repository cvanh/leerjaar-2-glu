...
const unit = (unit2) => (value) => util.num(value) && value !== 0 ? `${value}${unit2}` : value;
const ms = unit("ms");
const px$1 = unit("px");
const deg = unit("deg");
...
const rpx = (value, options) => {
  if (!util.string(value) || value.length < 4)
    return value;
...
const transformNegative = (_, { rawValue, variants, props }) => {
  if (util.string(rawValue)) {
    ...
    const varVal = util.getThemeValue(props, abs, variants);
    const value = util.string(varVal) || util.num(varVal) ? varVal : abs;
    return neg ? `-${value}` : value;
  }
  if (util.num(rawValue)) {
    const neg = util.negative(rawValue);
    const abs = Math.abs(rawValue);
    const varVal = variants ? variants[abs] : void 0;
    if (util.string(varVal))
      return neg ? `-${varVal}` : varVal;
    const value = util.num(varVal) ? varVal : abs;
...
const getVariants = (props) => {
  ...
  return __spreadValues$6(__spreadValues$6({}, medias), states);
};
...
const themeGetter = ({
  ...
  const getter = (value, defaultValue) => (props) => {
    let res = value;
    if (!util.string(value) && !util.num(value) && value !== true) {
      return res;
    }
    ...
    const getValue = (value2) => {
      const localDefaultValue = util.is(defaultValue) ? defaultValue : value2;
      let res2 = value2;
      const variants = util.is(key) ? util.getThemeValue(props, key) : null;
      if (util.is(variants)) {
        const path = value2 === true ? "default" : util.string(value2) || util.num(value2) ? value2 : null;
        if (util.is(path)) {
          const fromTheme = util.getThemeValue(props, path, variants);
...
const reduceVariants = (props, values, getStyle) => {
  ...
  for (const value in values) {
    ...
    if (state === void 0)
      continue;
    if (state === null) {
      styles = util.merge(styles, style2);
    } else {
      styles[state] = styles[state] ? util.assign(styles[state], style2) : style2;
    }
  }
  return styles;
};
...
const compose = (...generators) => {
  let flatGenerators = [];
  generators.forEach((gen) => {
    util.warn(Boolean(gen), `Undefined generator in "compose" method`);
    if (!gen)
      return;
...
const all = compose(fontFamily, fontSize, fontStyle, lineHeight, fontWeight, textAlign, letterSpacing, color, textTransform, textDecoration, verticalAlign, whiteSpace, textOverflow, listStyleType, listStylePosition);
const text = style({
  prop: "text",
  key: "texts",
...
const th = (path, defaultValue) => (props) => {
  const value = util.getThemeValue(props, path);
  if (util.is(value))
    return value;
  if (util.is(defaultValue))
    return defaultValue;
  return path;
};
[
...
const rpxTransformers = {
  ...
  border: (value) => util.num(value) && value > 0 ? `${remPx(value)} solid` : value
};
...
const generateHexAlphaVariants = (colors, variants = defaultAlphaVariants) => {
  ...
  const alphaColors = Object.keys(colors).reduce((obj, key) => {
    variants.forEach((variant) => {
      ...
      obj[variantKey] = util.string(value) ? transform(value, variant) : generateHexAlphaVariants(value, variants);
    });
    return obj;
  }, {});
